...
# print("Requesting user input for value of k...")
k = float(input("Please enter the value of k you would like to use for training the model: "))


# print("k value retrieved from user.")
...
def calc_bigram_probs():
    for x in bigram_counts:
        ...
        for y in bigram_counts[x]:
            # addk function had different probs for bigrams starting with <s> vs not
            if x == '<s>':
                bigram_probs[x][y] = (bigram_counts[x][y] + k) / (unigram_counts[x] + len(unigram_counts) - 2)
            else:
                bigram_probs[x][y] = (bigram_counts[x][y] + k) / (unigram_counts[x] + len(unigram_counts) - 1)
...
def get_bigram_prob(bigram):
    ...
    if (x in bigram_probs) and (y in bigram_probs[x]):
        ...
        return 1 / (unigram_counts[x] + len(unigram_counts) - 1)
